...
class VGNet_Feature(nn.Module):
    def __init__(self):
        ...
        self.xyz_res1 = nn.Sequential(nn.Conv1d(16,16,1),nn.LeakyReLU(),nn.Conv1d(16,16,1),nn.LeakyReLU())
        self.xyz_conv12 = torch.nn.Conv1d(16, 32, 1)
        self.xyz_res2 = nn.Sequential(nn.Conv1d(32,32,1),nn.LeakyReLU(),nn.Conv1d(32,32,1),nn.LeakyReLU())
        self.xyz_conv13 = torch.nn.Conv1d(32, 64, 1)
        self.xyz_mlp = torch.nn.Linear(230,300)
        self.xyz_conv21 = torch.nn.Conv1d(300, 224, 16)
        self.xyz_conv22 = torch.nn.Conv1d(224, 112, 16)
        self.xyz_conv23 = torch.nn.Conv1d(112, 64, 16)


        self.rgb_conv11 = torch.nn.Conv1d(3, 16, 1)
        self.rgb_res1 = nn.Sequential(nn.Conv1d(16,16,1),nn.LeakyReLU(),nn.Conv1d(16,16,1),nn.LeakyReLU())
        self.rgb_conv12 = torch.nn.Conv1d(16, 32, 1)
        self.rgb_res2 = nn.Sequential(nn.Conv1d(32,32,1),nn.LeakyReLU(),nn.Conv1d(32,32,1),nn.LeakyReLU())
        self.rgb_conv13 = torch.nn.Conv1d(32, 64, 1)
        self.rgb_mlp = torch.nn.Linear(230,300)
        self.rgb_conv21 = torch.nn.Conv1d(300, 224, 16)
        self.rgb_conv22 = torch.nn.Conv1d(224, 112, 16)
        self.rgb_conv23 = torch.nn.Conv1d(112, 64, 16)
    ...
    def forward(self, x, r, normal, view_center):                          # x = xyz, r = rgb, normal = normals
        ...
        x1_res = x1 + self.rgb_res1(x1)
        x2 = F.leaky_relu(self.xyz_conv12(x1_res))              # Conv1d(16,32)
        x2_res = x2 + self.xyz_res2(x2)
        x3 = F.leaky_relu(self.xyz_conv13(x2_res))                  # Conv1d(32,64)
        ...
        r1_res = r1 + self.rgb_res1(r1)
        r2 = F.leaky_relu(self.rgb_conv12(r1_res))              # Conv1d(16,32)
        r2_res = r2 + self.rgb_res2(r2)
        r3 = F.leaky_relu(self.rgb_conv13(r2_res))                  # Conv1d(32,64)
        ...
        concat1 = torch.cat((concat1,view_center,normal),dim = 1).permute(0,2,1)
        r_concat1 = torch.cat((r_concat1,view_center,normal), dim=1).permute(0,2,1)

        concat1 = self.xyz_mlp(concat1).permute(0,2,1)                     # GlobalFeature    BxCxN = 1x300xN
        r_concat1 = self.rgb_mlp(r_concat1).permute(0,2,1)                 # GlobalFeature    BxCxN = 1x300xN


        x1 = self.xyz_conv21(concat1)
        x2 = self.xyz_conv22(x1)
        x3 = self.xyz_conv23(x2)
        x1 = torch.max(x1, 2, keepdim=True)[0]        # PointFeature
        x2 = torch.max(x2, 2, keepdim=True)[0]        # PointFeature
        x3 = torch.max(x3, 2, keepdim=True)[0]        # PointFeature
        xyz = torch.cat((x1,x2,x3), dim=1).permute(0,2,1)

        r1 = self.rgb_conv21(r_concat1)
        r2 = self.rgb_conv22(r1)
        r3 = self.rgb_conv23(r2)
        r1 = torch.max(r1, 2, keepdim=True)[0]        # PointFeature
        r2 = torch.max(r2, 2, keepdim=True)[0]        # PointFeature
        r3 = torch.max(r3, 2, keepdim=True)[0]        # PointFeature
        rgb = torch.cat((r1,r2,r3), dim=1).permute(0,2,1)
...
class VGNet_XYZ(nn.Module):
    def __init__(self):
        ...
        self.mlp31 = torch.nn.Linear(800, 256)
...
class view_generate(nn.Module):
    def __init__(self):
        ...
        self.attention1 = VGNet_SelfAttention(400)
        self.attention2 = VGNet_SelfAttention(400)
